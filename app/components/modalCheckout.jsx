...
export default function ModalCheckOut({ products, showModal, setShowModal, subTotal }) {
    ...
    async function handleClick() {
        ...
        if(!handleEmailChange() || !handleNameChange()){
            return;
        }
                ...
                <Overlay handleModalClose={handleModalClose}>
                    <div onClick={(e) => e.stopPropagation()}
                        ...
                        {success ? <EmailSuccess /> : <>
                            {loading ? (<Loading />) : (
                                <>
                                    {error ? (<EmailError handleModalClose={handleModalClose} setError={setError} errorMessage={errorMessage} />) : (
                                        <>
                                            {emailSent ? (
                                                ...
                                                <div className="flex flex-row h-3/4 w-full mb-4">
                                                    <form className="flex flex-col font-medium items-center justify-center w-1/2">
                                                        <div className="w-3/4 mb-4 ">
                                                            ...
                                                            <input onChange={handleNameChange} ref={nameRef} className="w-full h-9 border-[1.5px] rounded-3xl pl-4 py-5 border-black border-solid " placeholder="Name: " />
                                                        </div>
                                                        <div className="w-3/4">
                                                            <div className="flex flex-row justify-between">
                                                            ...
                                                            <input onChange={handleEmailChange} ref={emailRef} className="w-full h-9 rounded-3xl pl-4 py-5 border-[1.5px] border-black border-solid" placeholder="Email: " />
                                                        </div>
                                                    </form>
